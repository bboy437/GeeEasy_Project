...
function sort(countries: IProductGroup[], column: string, direction: string): IProductGroup[] {
  if (direction === '') {
    return countries;
  } else {
...
function matches(country: IProductGroup, term: string, pipe: PipeTransform) {
  return country.inventory_group_name.toString().toLowerCase().includes(term.toString().toLowerCase())
...
export class ProductGroupTableService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _countries$ = new BehaviorSubject<IProductGroup[]>([]);
