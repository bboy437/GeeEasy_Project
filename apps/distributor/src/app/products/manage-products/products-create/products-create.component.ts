...
export class ProductsCreateComponent implements OnInit {
  ...
  private URLRputer_ProductDetail =
    "products/manage/detail-product-distributor";
  arrobjRow: any = [];
  arrobjCategory: any = [];
  arrWarehouse: any = [];
  ...
  editForm() {
    ...
    console.log(
      ...
      this.arrobjRow.product_category_array
    );

    this.productForm.patchValue({
      productCategory: this.arrobjRow.product_category_array.length === 0 ? "" : this.arrobjRow.product_category_array[0].product_category_id,
      ...
      initial_stock: this.arrobjRow.product_warehouse_array[0].onhand,
  ...
  getCategory() {
    ...
    this.browseSupplierAPIService.getCategory(value).subscribe(data => {
      ...
      if (this.RowID !== "new") {
        this.strUpdateName = this.arrobjRow.product_category_array.length === 0 ? "" : this.arrobjRow.product_category_array[0].product_category_name;
        setTimeout(() => {
          this.productForm.get("productCategory").patchValue(this.arrobjRow.product_category_array.length === 0 ? "" : +this.arrobjRow.product_category_array[0].product_category_id);
          this.product_category__name = this.arrobjRow.product_category_array.length === 0 ? "" : this.arrobjRow.product_category_array[0].product_category_name;
    ...
    this.warehouseAPIService.getWarehouseList(value).subscribe(data => {
      ...
      if (this.RowID !== "new") {
        setTimeout(() => {
          ...
          this.productForm.get("warehouse_name").disable();
        }, 0);
      }
      ...
      .getImageArrayState(this.product.product_image_array.get, state => {
        ...
        if (this.RowID === "new") {
          this.productAPIService
            .postSKUDist(JSON.stringify(dataJson))
            .subscribe(res => {
              console.log(res);
          ...
          console.log("productForm", this.productForm.value.sku);
          console.log("productForm", this.productForm.value.productSKU);
          if (
  ...
  save() {
    ...
    if (this.RowID === "new") {
      ...
      this.productAPIService
        .addProductsDistributor(JSON.stringify(dataJson))
        .subscribe(data => {
          console.log(data);
      ...
      const dataJson = {
        distributor_product_id: this.arrobjRow.distributor_product_id,
        distributor_id: this.id_local,
        product_image_url:
          this.product.main_image.port.length > 0
            ? this.product.main_image.port[0].image_url
        ...
        product_warehouse_array: [
          {
            warehouse_id: +this.arrobjRow.product_warehouse_array[0].warehouse_id,
            warehouse_name: this.arrobjRow.product_warehouse_array[0].warehouse_name,
            onhand: +this.productForm.value.initial_stock,
            available: 0,
            incoming: 0,
            outgoing: 0,
  ...
  btnBackClick() {
    ...
    dialogRef.onClose.subscribe(result => {
      ...
      if (result === "ok") {
        this.router.navigate([this.URLRputer_ProductDetail, this.RowID, this.status]);
      }
    });
  }
