...
function matches(country: ISalerepAccount, term: string, pipe: PipeTransform) {
  return country.create_time.toString().toLowerCase().includes(term.toString().toLowerCase())
    ...
    || country.sale_rep_addr_amphoe.toString().toLowerCase().includes(term.toString().toLowerCase())
    || country.sale_rep_addr_province.toString().toLowerCase().includes(term.toString().toLowerCase())
}
...
export class WishlistTableService {
  ...
  getData(callback: (res, countries$, total$) => any) {
    this.saleRepService.salerepListDisributor$.subscribe(res => {
      this.arrWishlist = res;
      console.log("getData : arrWishlist : ", res);
