...
@Component({
  selector: 'project-purchase-reorder',
  templateUrl: './purchase-reorder.component.html',
  styleUrls: ['./purchase-reorder.component.scss']
})
export class PurchaseReorderComponent implements OnInit {
  ...
  getPurchaseDetail() {
    this.purchaseAPIService.getPurchaseDetail(this.RowID).subscribe(data => {
      ...
      this.purchase_order_product_array = data.response_data[0].purchase_order_product_array;
      this.supplierID = String(data.response_data[0].supplier_id);
      console.log('arrPurchase', this.arrPurchase);
  ...
  getDisabledDates() {
    ...
    let endDate: Date = new Date();//change as per your need
  ...
  getSupplier() {
    ...
    this.supplierAPIService.getVerifiedSupplieList(value).subscribe(data => {
      ...
      this.Form.get('SupplierName').patchValue(this.arrPurchase.supplier_id);
      this.Form.get('SupplierName1').patchValue(this.arrPurchase.supplier_id);
      this.getProductFromReorder();
    })
  }
  ...
  getWarehouse() {
    ...
    this.warehouseAPIService.getWarehouseList(value).subscribe(data => {
      ...
      this.Form.get('WarehouseName').patchValue(this.arrPurchase.warehouse_id);
      this.Form.get('Note').patchValue(this.arrPurchase.purchase_order_note);
      this.Form.get('BillingTerm').patchValue(this.arrPurchase.billing_payment_term);
    })
  }

  getProductFromReorder() {
    this.supplierAPIService.getSupID(this.arrPurchase.supplier_id).subscribe(res => {
      this.arrSupplierLists = res.response_data;
      this.getProduct(this.arrSupplierLists[0].supplier_product_array)
    })
    this.getProductGroup();
    this.getSetting();
    this.getProductReorder();
  }
  ...
  getProductReorder() {
    ...
    this.purchase_order_product_array.forEach(element => {
      this.products.push(
        this.fb.group({
          product_name: element.request_product_name,
          product_sku: element.confirm_product_sku,
          product_qty: element.confirm_product_qty,
          product_unit: element.product_data.product_unit,
          product_price: element.request_product_price,
          product_image_url: element.request_product_image_url,
          product_wholesale_array: this.fb.array(element.product_data.product_data.product_wholesale_array),
          product_totalplice: element.request_product_price,
          supplier_product_id: element.product_data.product_id,
          product_attribute: element.product_data.product_data.product_attribute,
          product_currency_code: element.product_data.product_data.product_currency_code,
          isCheckPlice: true,
          numSales: element.request_product_price,
    ...
    this.currencyTotal = this.purchase_order_product_array[0].product_data.product_data.product_currency_code;
    this.sumTotal();
    this.checkCurrency();
