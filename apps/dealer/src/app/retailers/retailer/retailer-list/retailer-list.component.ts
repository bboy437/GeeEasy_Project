...
export class RetailerListComponent implements OnInit {
    private create = "retailers/retailer/create";
    private detail = "retailers/retailer/detail";

    strFilter: string;

    wishlist$: Observable<IRetailer[]>;
    ...
    btnRow(item) {
        ...
        if (item.type == "click") {
            ...
            this.router.navigate([this.detail, item.row.retail_id]);
        }
    };
