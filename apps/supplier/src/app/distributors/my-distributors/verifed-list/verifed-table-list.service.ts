...
function sort(countries: IDistVerifed[], column: string, direction: string): IDistVerifed[] {
  if (direction === '') {
    return countries;
  } else {
...
function matches(country: IDistVerifed, term: string, pipe: PipeTransform) {
  return country.toString().toLowerCase().includes(term.toString().toLowerCase())
...
export class VerifedTableService {
  ...
  private _countries$ = new BehaviorSubject<IDistVerifed[]>([]);
  ...
  getData(callback) {
    ...
    // const value = "cur_page=" + 1 + "&per_page=" + 10 + "&supplier_id=" + 8181569;
    ...
    this.distributorAPIService.getVerifiedCate(supplier_id).subscribe(data => {
      this.arrVerifed = data.response_data;
      console.log(this.arrVerifed);
