...
@Component({
  selector: "project-display-file-image",
  templateUrl: "./display-file-image.component.html",
  styleUrls: ["./display-file-image.component.scss"]
})
export class DisplayFileImageComponent implements OnInit {
  @Input() theme = "white";
  @Input() height = "0";
          ...
          for (const item_files of event.target.files) {
            ...
            reader.onload = (red: any) => {
              function Object() {
                ...
                this.create_time_string = new Date(
                  Date.now()
                ).toLocaleDateString();
                this.update_time = ~~(Date.now() / 1000);
                this.update_time_string = new Date(
                  Date.now()
                ).toLocaleDateString();
              }
